...
print met_trigger_columns['HLT_PFMET170_NoiseCleaned']
...
tau_columns = {'pt':'Tau_pt','eta':'Tau_eta','phi':'Tau_phi','mass':'Tau_mass','decayMode':'Tau_idDecayMode','decayModeNew':'Tau_idDecayModeNewDMs','id':'Tau_idMVAnew'} # (nesDM does not exist in my file)
...
for arrays in uproot.iterate(f,'Events',columns,entrysteps=50000):
        ...
        triggers = JaggedArray.fromcounts(np.ones(50000, dtype = int), Table(**{'MET':np.prod([arrays[val] for val in met_trigger_columns]),'SingleEle':np.prod([arrays[val] for val in singleele_trigger_columns]),'SinglePhoton':np.prod([arrays[val] for val in singlephoton_trigger_columns])}))
